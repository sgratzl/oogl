...
#define GLM_FORMATTING std::setw(5) << std::setprecision(3) << std::fixed << std::setfill('0') << std::right

template<typename U>
std::ostream &operator<<(std::ostream &ostr, const glm::detail::tvec1<U> &v) {
	return ostr << GLM_FORMATTING << std::fixed
			<< "(" << v.x << ")";
}
...
std::ostream &operator<<(std::ostream &ostr, const glm::detail::tmat2x2<T> &v) {
	...
	ostr << "[" << GLM_FORMATTING << std::fixed << row.x << "," << row.y << " " << std::endl;
	row = glmm::row(v,1);
	ostr << " " << GLM_FORMATTING << std::fixed << row.x << "," << row.y << "]" << std::endl;
	return ostr;
}
...
std::ostream &operator<<(std::ostream &ostr, const glm::detail::tmat3x3<T> &v) {
	...
	ostr << "[" << GLM_FORMATTING << std::fixed<< row.x << "," << row.y << "," << row.z << " " << std::endl;
	row = glmm::row(v,1);
	ostr << " " << GLM_FORMATTING << std::fixed<< row.x << "," << row.y << "," << row.z << " " << std::endl;
	row = glmm::row(v,2);
	ostr << " " << GLM_FORMATTING << std::fixed << row.x << "," << row.y << "," << row.z << "]" << std::endl;
...
std::ostream &operator<<(std::ostream &ostr, const glm::detail::tmat4x4<T> &v) {
	...
	ostr << "[" << GLM_FORMATTING << std::fixed<< row.x << "," << row.y << "," << row.z << "," << row.w << " " << std::endl;
	row = glmm::row(v,1);
	ostr << " " << GLM_FORMATTING << std::fixed<< row.x << "," << row.y << "," << row.z << "," << row.w << " " << std::endl;
	row = glmm::row(v,2);
	ostr << " " << GLM_FORMATTING << std::fixed<< row.x << "," << row.y << "," << row.z << "," << row.w << "]" << std::endl;
